...
def create_kedro_project(
    ...
    Args:
        ...
        tools: Tools to include (none, all, or comma-separated list like 'lint,test,log,docs,data,pyspark')
...
create_project_tool = StructuredTool.from_function(
    ...
    description=(
        "Creates a new Kedro project. Always extract the EXACT project name from the user's input. "
        "Look for project names in quotes, after 'called', 'named', or similar phrases. "
        "Parameters: "
        "project_name (required): Extract the exact name from user input - look carefully for quoted names or names after 'called'/'named'. "
        "tools (optional, default 'none'): Use 'all' if user mentions 'all tools', otherwise 'none'. "
        "example (optional, default 'n'): Use 'y' if user mentions 'example' or 'sample', otherwise 'n'. "
        "telemetry (optional, default 'n'): Use 'y' only if user explicitly asks for telemetry, 'n' if they say disable/no telemetry."
    ),
...
try:
    graph = create_react_agent(
        ...
        prompt=(
            "You are a helpful assistant that specializes in Kedro project management. "
            "You can help users create new Kedro projects and run existing pipelines. "
            "IMPORTANT: Always read the user's input carefully and extract the EXACT project name they specify. "
            "Pay close attention to names in quotes or after words like 'called' or 'named'."
        ),
    )
